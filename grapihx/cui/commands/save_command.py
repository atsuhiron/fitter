...
class SaveCommand(BaseCommand):
    ...
    def get_command_type(cls) -> CuiMainCommandType:
        return CuiMainCommandType.SAVE
    ...
    def check(self):
        if len(self.com_args) != 1:
            raise CommandParseException("ファイル名を指定してください: {}".format(self.com_args))

        if not isinstance(self.com_args[0], str):
            # function name must be string
            raise CommandParseException("ファイル名は文字列で指定してください: {}".format(self.com_args))
